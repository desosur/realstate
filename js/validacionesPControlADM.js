...
{	
    ...
    if (rut.length < 9) {
		alert("Ingrese un RUT de al menos 9 dígitos.");
		$("#txtRut").val("");
        return false;
    } else {
        i1 = rut.indexOf("-");
        ...
        if (dvokstr == dv) {            
			...
			alert("RUT inv&aacute;lido, ingr&eacute;selo correctamente para continuar.");
			$("#txtRut").val("");
            return false;
        }
    }
...
function validaEmail() {
	...
	if( !emailReg.test(email) ) {
		alert("El e-mail ingresado es inv&aacute;lido, ingr&eacute;selo correctamente para continuar.");
		$("#txtEmail").val("");
		return false;
	} else {
		return true;
