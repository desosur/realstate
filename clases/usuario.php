...
class usuario
{
	private $getTodoAuditUsers, $getTodoContacto, $getDatosUser, $getDatosUserTodos, $getDatosUserAny, $getLucesUser, $getSensoresUser, $getCompsUser, $getUsoUser = array();
	...
	public function buscar_usuario_any($term)
	{
		//
		$control = true;		
		$str = "select ID_CLIENTE, RUT, NOMBRES, APE_PATERNO, APE_MATERNO, DATE_FORMAT(FECHA_NAC, '%d-%m-%Y') AS FECHA_NAC, GENERO, COMUNA, CALLE, FONO_MOVIL, FONO_FIJO, EMAIL, USER_CLIE from cliente where ID_CLIENTE = '$term' OR RUT = '$term' OR USER_CLIE = '$term' not like '%admin%' OR NOMBRES LIKE '%$term%' OR APE_PATERNO LIKE '%$term%' OR APE_MATERNO LIKE '%$term%';";
		$ejecutar = mysql_query($str) or die(mysql_error());
		while($resultado = mysql_fetch_assoc($ejecutar))	{
	...
	{
		$str = "select ID_CLIENTE, RUT, NOMBRES, APE_PATERNO, APE_MATERNO, DATE_FORMAT(FECHA_NAC, '%d-%m-%Y') AS FECHA_NAC, GENERO, COMUNA, CALLE, FONO_MOVIL, FONO_FIJO, EMAIL, USER_CLIE, AES_DECRYPT(PASS_CLIE,'llave_AES') AS PASS_CLIE from cliente where ID_CLIENTE = '$term' OR RUT = '$term' OR USER_CLIE = '$term' not like '%admin%'";
		$ejecutar = mysql_query($str) or die(mysql_error());
		while($resultado = mysql_fetch_assoc($ejecutar))	{
	...
	{
		$str = "select * from cliente where ID_CLIENTE not like 1;";
		$ejecutar = mysql_query($str) or die(mysql_error());
		while($resultado = mysql_fetch_assoc($ejecutar))	{
	...
	public function listar_componentes_user($id_clie)
	{
		$str = "select * from componente where ID_ARD in (select ID_ARDUINO from arduino where ID_RASPBE in(select ID_RASPBERRY from raspberry where ID_CLIEN = $id_clie));";
		$ejecutar = mysql_query($str) or die(mysql_error());
		while($resultado = mysql_fetch_assoc($ejecutar))	{
			$this->getCompsUser[]=$resultado;
		}
		return $this->getCompsUser;
	}
	
	public function listar_uso_user($id_clie)
	{	
		$str2 = "select * from uso where FECH_HOR_FIN not like 'null' and ID_COMP in (select ID_COMPONENTE from componente where ID_ARD in (select ID_ARDUINO from arduino where ID_RASPBE in(select ID_RASPBERRY from raspberry where ID_CLIEN = $id_clie)));";
		$ejecutar2 = mysql_query($str2) or die(mysql_error());
		while($resultado = mysql_fetch_assoc($ejecutar2))	{
			$this->getUsoUser[]=$resultado;
		}
		return $this->getUsoUser;
	}
	...
	public function login_usuario($user, $pass, $platform)
	{
		...
		if ($num_reg == 0)
		{
			...
			if ($pass_obtenida == $pass) {
				...
				$_SESSION["Expire"] = $_SESSION["Set_Timeout"] + (120 * 60);		// cant_minutos * 60
	...
	public function cuenta_login_smartphone() {
		...
		$control = true;
		$cant_smrt = 0;
		//$str = "select count(ID_REG_CLIE) as CANT_SMRT from audit_cliente where COMENTARIO_CLIE not like '%ha Cerrado%' and COMENTARIO_CLIE not like '%ha sido registrada%' and COMENTARIO_CLIE not like '%ha actualizado%' and COMENTARIO_CLIE not like '%ha sido eliminado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%' and COMENTARIO_CLIE like '%Smartphone%';";
		$str = "select count(ID_REG_CLIE) as CANT_SMRT from audit_cliente where COMENTARIO_CLIE like '%ha Iniciado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%' and COMENTARIO_CLIE like '%Smartphone%';";
		$ejecutar = mysql_query($str) or die(mysql_error());
		$num_reg = mysql_num_rows($ejecutar);
		if ($num_reg == 0)
		{
	...
	public function cuenta_login_tablet() {
		...
		$control = true;
		$cant_tabl = 0;
		//$str = "select count(ID_REG_CLIE) as CANT_TABL from audit_cliente where COMENTARIO_CLIE not like '%ha Cerrado%' and COMENTARIO_CLIE not like '%ha sido registrada%' and COMENTARIO_CLIE not like '%ha actualizado%' and COMENTARIO_CLIE not like '%ha sido eliminado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%' and COMENTARIO_CLIE like '%Tablet%';";
		$str = "select count(ID_REG_CLIE) as CANT_TABL from audit_cliente where COMENTARIO_CLIE like '%ha Iniciado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%' and COMENTARIO_CLIE like '%Tablet%';";
		$ejecutar = mysql_query($str) or die(mysql_error());
		$num_reg = mysql_num_rows($ejecutar);
		if ($num_reg == 0)
		{
	...
	public function cuenta_login_noche()
	{
		// Se excluye al admin
		$control = true;
		$cant_noche = 0;
		$str = "select count(ID_REG_CLIE) as CANT_NOCHE from audit_cliente where DATE_FORMAT(FECH_HOR_CLIE, '%H:%i:%s') between '00:00:00' and '07:59:59' and COMENTARIO_CLIE like '%ha Iniciado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%'";
		$ejecutar = mysql_query($str) or die(mysql_error());
		$num_reg = mysql_num_rows($ejecutar);
		if ($num_reg == 0)
		{
			...
			$arreglo = mysql_fetch_array($ejecutar);
			$cant_noche = $arreglo["CANT_NOCHE"];
			return $cant_noche;
		}
		
	}
	
	public function cuenta_login_dia()
	{
		// Se excluye al admin
		$control = true;
		$cant_dia = 0;
		$str = "select count(ID_REG_CLIE) as CANT_DIA from audit_cliente where DATE_FORMAT(FECH_HOR_CLIE, '%H:%i:%s') between '08:00:00' and '15:59:59' and COMENTARIO_CLIE like '%ha Iniciado%' and COMENTARIO_CLIE not like '%admin%' and COMENTARIO_CLIE not like '%Administrador%'";
		$ejecutar = mysql_query($str) or die(mysql_error());
		$num_reg = mysql_num_rows($ejecutar);
		if ($num_reg == 0)
		{
			...
			$arreglo = mysql_fetch_array($ejecutar);
			$cant_dia = $arreglo["CANT_DIA"];
			return $cant_dia;
		}
		
	}
	
	public function cuenta_login_tarde()
	{
		// Se excluye al admin
		$control = true;
		$cant_tarde = 0;
